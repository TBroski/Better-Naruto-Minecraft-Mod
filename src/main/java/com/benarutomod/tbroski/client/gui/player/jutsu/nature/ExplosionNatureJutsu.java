...
public class ExplosionNatureJutsu extends AbstractJutsuScreen {

    private ArrayList<BeNMJutsu> jutsus = new ArrayList<>();

    public ExplosionNatureJutsu() {
        super(new TranslationTextComponent("gui." + Main.MODID + ".title.explosionnaturejutsu"));
        for (BeNMJutsu jutsu : BeNMRegistry.JUTSUS.getValues()) {
            if (jutsu.getType() == BeNMJutsu.Type.EXPLOSION_NATURE) {
