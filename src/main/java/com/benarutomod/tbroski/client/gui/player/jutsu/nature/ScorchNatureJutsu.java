...
public class ScorchNatureJutsu extends AbstractJutsuScreen {

    private ArrayList<BeNMJutsu> jutsus = new ArrayList<>();

    public ScorchNatureJutsu() {
        super(new TranslationTextComponent("gui." + Main.MODID + ".title.scorchnaturejutsu"));
        for (BeNMJutsu jutsu : BeNMRegistry.JUTSUS.getValues()) {
            if (jutsu.getType() == BeNMJutsu.Type.SCORCH_NATURE) {
