...
public class IceNatureJutsu extends AbstractJutsuScreen {

    private ArrayList<BeNMJutsu> jutsus = new ArrayList<>();

    public IceNatureJutsu() {
        super(new TranslationTextComponent("gui." + Main.MODID + ".title.icenaturejutsu"));
        for (BeNMJutsu jutsu : BeNMRegistry.JUTSUS.getValues()) {
            if (jutsu.getType() == BeNMJutsu.Type.ICE_NATURE) {
