...
public class StormNatureJutsu extends AbstractJutsuScreen {

    private ArrayList<BeNMJutsu> jutsus = new ArrayList<>();

    public StormNatureJutsu() {
        super(new TranslationTextComponent("gui." + Main.MODID + ".title.stormnaturejutsu"));
        for (BeNMJutsu jutsu : BeNMRegistry.JUTSUS.getValues()) {
            if (jutsu.getType() == BeNMJutsu.Type.STORM_NATURE) {
