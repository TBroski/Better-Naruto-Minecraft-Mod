...
public class BoilNatureJutsu extends AbstractJutsuScreen {

    private ArrayList<BeNMJutsu> jutsus = new ArrayList<>();

    public BoilNatureJutsu() {
        super(new TranslationTextComponent("gui." + Main.MODID + ".title.boilnaturejutsu"));
        for (BeNMJutsu jutsu : BeNMRegistry.JUTSUS.getValues()) {
            if (jutsu.getType() == BeNMJutsu.Type.BOIL_NATURE) {
