...
public class LightningNatureJutsu extends Screen {
    ...
    Button buttonKey8;
    Button buttonKey9;
    GuiButtonJutsu guiButtonLightningBallJutsu;
    GuiButtonJutsu guiButtonStunGunJutsu;
    GuiButtonJutsu guiButtonLightningArrowJutsu;
    private int lightningBallCost = 4;
    private int stunGunCost = 6;
    private int lightningArrowCost = 5;


    public LightningNatureJutsu() {
        super(new TranslationTextComponent("gui." + Main.MODID + ".title.lightningnaturejutsu"));
        this.guiTitle = new TranslationTextComponent("gui." + Main.MODID + ".title.lightningnaturejutsu");
    }
    ...
    public void render(int p_render_1_, int p_render_2_, float p_render_3_) {
        ...
        guiButtonLightningBallJutsu.renderButton(p_render_1_, p_render_2_, p_render_3_);
        guiButtonStunGunJutsu.renderButton(p_render_1_, p_render_2_, p_render_3_);
        guiButtonLightningArrowJutsu.renderButton(p_render_1_, p_render_2_, p_render_3_);
    ...
    protected void init() {
        ...
        addButton(guiButtonLightningBallJutsu = new GuiButtonJutsu(this.guiLeft - 90, this.guiTop - 90, 48, 0, $ -> {
            if (playerc.hasLightningBallJutsuBoolean())
            {
                this.jutsuToggle = "jutsu." + Main.MODID + ".lightningball";
            }
            else if (playerc.returnBeNMPoints() >= this.lightningBallCost)
            {
                playerc.addBeNMPoints(-this.lightningBallCost);
                playerc.setLightningBallJutsuBoolean(true);
                NetworkLoader.INSTANCE.sendToServer(new PacketBeNMPointsSync(playerc.returnBeNMPoints(), false));
                NetworkLoader.INSTANCE.sendToServer(new PacketSetJutsuBoolean(ShootingJutsu.LightningBallJutsuID, playerc.hasLightningBallJutsuBoolean(), false));

            }
            else {
                player.sendMessage(new StringTextComponent("Not Enough BeNM Points (Need " + this.lightningBallCost + ")"));
            }
        }));

        addButton(guiButtonStunGunJutsu = new GuiButtonJutsu(this.guiLeft - 70, this.guiTop - 90, 48, 16, $ -> {
            if (playerc.hasStunGunJutsuBoolean())
            {
                this.jutsuToggle = "jutsu." + Main.MODID + ".stungun";
            }
            else if (playerc.returnBeNMPoints() >= this.stunGunCost)
            {
                playerc.addBeNMPoints(-this.stunGunCost);
                playerc.setStunGunJutsuBoolean(true);
                NetworkLoader.INSTANCE.sendToServer(new PacketBeNMPointsSync(playerc.returnBeNMPoints(), false));
                NetworkLoader.INSTANCE.sendToServer(new PacketSetJutsuBoolean(ShootingJutsu.StunGunJutsuID, playerc.hasStunGunJutsuBoolean(), false));

            }
            else {
                player.sendMessage(new StringTextComponent("Not Enough BeNM Points (Need " + this.stunGunCost + ")"));
            }
        }));

        addButton(guiButtonLightningArrowJutsu = new GuiButtonJutsu(this.guiLeft - 50, this.guiTop - 90, 48, 32, $ -> {
            if (playerc.hasLightningArrowJutsuBoolean())
            {
                this.jutsuToggle = "jutsu." + Main.MODID + ".lightningarrow";
            }
            else if (playerc.returnBeNMPoints() >= this.lightningArrowCost)
            {
                playerc.addBeNMPoints(-this.lightningArrowCost);
                playerc.setLightningArrowJutsuBoolean(true);
                NetworkLoader.INSTANCE.sendToServer(new PacketBeNMPointsSync(playerc.returnBeNMPoints(), false));
                NetworkLoader.INSTANCE.sendToServer(new PacketSetJutsuBoolean(ShootingJutsu.LightningArrowJutsuID, playerc.hasLightningArrowJutsuBoolean(), false));

            }
            else {
                player.sendMessage(new StringTextComponent("Not Enough BeNM Points (Need " + this.lightningArrowCost + ")"));
            }
        }));
    ...
    {
        if (guiButtonLightningBallJutsu.isHovered())
        {
            renderTooltip(new TranslationTextComponent("jutsu." + Main.MODID + ".lightningball").getString(), p_render_1, p_render_2);
        }
        if (guiButtonStunGunJutsu.isHovered())
        {
            renderTooltip(new TranslationTextComponent("jutsu." + Main.MODID + ".stungun").getString(), p_render_1, p_render_2);
        }
        if (guiButtonLightningArrowJutsu.isHovered())
        {
            renderTooltip(new TranslationTextComponent("jutsu." + Main.MODID + ".lightningarrow").getString(), p_render_1, p_render_2);
        }
    }
    ...
    public void checkToggled()
    {
        ...
        if (this.jutsuToggle.equalsIgnoreCase("jutsu." + Main.MODID + ".lightningball"))
        {
            mc.ingameGUI.blit(this.guiLeft - 90, this.guiTop - 90, 240, 224, 16,16);
        }
        if (this.jutsuToggle.equalsIgnoreCase("jutsu." + Main.MODID + ".stungun"))
        {
            mc.ingameGUI.blit(this.guiLeft - 70, this.guiTop - 90, 240, 224, 16,16);
        }
        if (this.jutsuToggle.equalsIgnoreCase("jutsu." + Main.MODID + ".lightningarrow"))
        {
