...
public class IceNatureJutsuInit {
    ...
    public static void registerIceJutsu(BeNMRegistry.JutsuRegistry jutsuRegistry, IBeNMPlugin pluginIn) {
        ...
        jutsuRegistry.register(new BeNMJutsu(pluginIn, "ice_prison", BeNMJutsu.Type.ICE_NATURE, 15, 270F, 160, 16, false, (playerIn, taijutsuModifier0, taijutsuModifier1, playerCapability) -> {
            if (!playerIn.world.isRemote) {
        ...
        jutsuRegistry.register(new BeNMJutsu(pluginIn, "one_horned_white_whale", BeNMJutsu.Type.ICE_NATURE, 6, 125F, 160, 32, false, (playerIn, taijutsuModifier0, taijutsuModifier1, playerCapability) -> {
            if (!playerIn.world.isRemote) {
        ...
        jutsuRegistry.register(new BeNMJutsu(pluginIn, "ten_thousand_ice_petals", BeNMJutsu.Type.ICE_NATURE, 18, 2F, 160, 48, true, (playerIn, taijutsuModifier0, taijutsuModifier1, playerCapability) -> {
            if (!playerIn.world.isRemote) {
                for (int i = 0; i < 1 + taijutsuModifier0; i++) {
                    ...
                    entity.shoot(playerIn, playerIn.rotationPitch + playerIn.getRNG().nextInt(i), playerIn.rotationYaw + playerIn.getRNG().nextInt(i), 0.0F, 0.8F + (playerIn.getRNG().nextInt(i) / 10), 5.0F);
                    playerIn.world.addEntity(entity);
                }
            }
        }));
