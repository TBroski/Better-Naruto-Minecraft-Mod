...
public class DojutsuSkullTileEntityRenderer extends TileEntityRenderer<DojutsuSkullTileEntity> {
    ...
    public static void render(@Nullable Direction directionIn, MatrixStack matrixStackIn, IRenderTypeBuffer buffer, int combinedLight, LivingEntity livingEntity) {
        ...
        if (livingEntity instanceof IDojutsuEntity) {
            ...
            AbstractShinobiEntity shinobiEntity = (AbstractShinobiEntity) livingEntity;
            ModelLeftEye modelLeftEye = new ModelLeftEye(shinobiEntity.getLeftDojustsu().getSize(), dojutsuEntity.eyeSlot() - 5);
            ModelRightEye modelRightEye = new ModelRightEye(shinobiEntity.getRightDojustsu().getSize(), dojutsuEntity.eyeSlot() - 5);
            if (directionIn == null) {
            ...
            IVertexBuilder ivertexbuilderlefteye = buffer.getBuffer(modelLeftEye.getRenderType(shinobiEntity.getLeftDojustsu().getResourceLocation()));
            if (shinobiEntity.getLeftDojustsu() != DojutsuInit.NULL)
                modelLeftEye.render(matrixStackIn, ivertexbuilderlefteye, combinedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
            IVertexBuilder ivertexbuilderrighteye = buffer.getBuffer(modelLeftEye.getRenderType(shinobiEntity.getRightDojustsu().getResourceLocation()));
            if (shinobiEntity.getRightDojustsu() != DojutsuInit.NULL)
                modelRightEye.render(matrixStackIn, ivertexbuilderrighteye, combinedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
        }
        matrixStackIn.pop();
    }
